...
sample_adx_data = read_csv("data/ADX_D.csv", index_col=0, parse_dates=True)
...
def load(**kwargs: DictLike):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("pandas-datareader is required for fallback download. " "Install the test extras or provide the CSV.") from ie
...
sample_data = load(fpath="data/SPY_D.csv", n=[-2 * _tdpy, -_tdpy, -200, -89, 0, 89, 200, _tdpy, 2 * _tdpy][4], verbose=VERBOSE)
